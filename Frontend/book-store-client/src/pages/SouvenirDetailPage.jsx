...
function SouvenirDetailPage() {
  ...
  const backHref = typeof location.state?.from === "string" ? location.state.from : "/souvenirs";
  ...
  const handleSubmitReview = async (e) => {
    ...
    try {
      const reviewPayload = {
        ...
        productTitle: product?.title || product?.name || "Quà lưu niệm",
        rating: tempRating,
        comment: comment
      };
    ...
    try {
      await apiRequest(`/carts/items`, {
        ...
        body: JSON.stringify({
          ...
          SouvenirId: Number(product.id || product.Id),
          Quantity: 1
        })
      });

      window.dispatchEvent(new Event('cartUpdated'));
      showToast(`Đã thêm "${product.title || product.name}" vào giỏ hàng!`, "success");
    } catch (err) {
  ...
  useEffect(() => {
    ...
    apiRequest(`/souvenirs/${idNum}`)
      ...
      .catch(() => {
        if (!cancelled) {
          setProduct(null);
          setError("Không tìm thấy sản phẩm hoặc không kết nối được máy chủ.");
        }
      })
      .finally(() => {
      ...
      <div className="books-page bg-light pb-5">
        <div className="container py-5">
          <div className="card border-0 shadow-sm rounded-4 p-5 text-center">
            ...
            <h5 className="fw-bold mb-0">Đang tải chi tiết quà lưu niệm...</h5>
          </div>
        </div>
      </div>
    );
        ...
        <div className="container py-4">
          <nav aria-label="breadcrumb" className="mb-4">
            <ol className="breadcrumb mb-0">
              ...
              <li className="breadcrumb-item"><Link to={backHref} className="text-decoration-none text-dark fw-medium">Quà lưu niệm</Link></li>
          ...
          <div className="card border-0 shadow-sm rounded-4 p-5 text-center">
            <i className="bi bi-gift display-4 text-muted mb-3" />
            <h3 className="fw-bold mb-2">{error || "Không tìm thấy sản phẩm hoặc không kết nối được máy chủ."}</h3>
            <p className="text-muted mb-4">URL: /souvenirs/{idParam}</p>
            <Link to={backHref} className="btn btn-success rounded-pill px-5 fw-bold">Về danh sách</Link>
          </div>
        </div>
      </div>
    );
  ...
  const { rating, reviews } = shopSignals(product);
  const title = product.title || product.name || "Sản phẩm quà lưu niệm";
  const stockQuantity = Number(product?.stockQuantity || 0);
  const outOfStock = stockQuantity === 0;
  const isAvailable = stockQuantity > 0;
  const categoryId = bookCategoryId(product);
  const categoryName = bookCategoryName(product) || "Quà lưu niệm";
  const brand = product.brand || product.manufacturer || "Đang cập nhật";
  ...
  return (
    <div className="books-page bg-light pb-5">
      <div className="container py-4">
        ...
        <button type="button" className="btn btn-link text-success fw-bold p-0 mb-4 text-decoration-none" onClick={() => navigate(backHref)}>
          <i className="bi bi-arrow-left me-2"></i> Quay lại danh sách quà lưu niệm
        </button>
          ...
          <div className="row g-0">
            <div className="col-lg-5 bg-white p-4 p-lg-5 border-end">
              <div className="position-relative">
                {product.imageUrl ? (
                  <img
                    src={getImageUrl(product.imageUrl, title)}
                    alt={title}
                ...
                {product.stockQuantity > 0 && product.stockQuantity <= 5 && (
                  <span className="position-absolute top-0 start-0 m-3 badge rounded-pill bg-warning text-dark fw-bold px-3 py-2 shadow-sm">Sắp hết · {product.stockQuantity}</span>
                )}
                {outOfStock && (
            ...
            <div className="col-lg-7 p-4 p-lg-5 bg-white">
              ...
              <div>
                <h4 className="fw-bold mb-3">Thông tin quà lưu niệm</h4>
                <p className="text-muted mb-0" style={{ lineHeight: 1.8, whiteSpace: "pre-wrap" }}>
                  {product.description || product.Description || "Sản phẩm quà lưu niệm tuyệt vời thuộc hệ thống nhà sách Book-Store."}
                </p>
              </div>
