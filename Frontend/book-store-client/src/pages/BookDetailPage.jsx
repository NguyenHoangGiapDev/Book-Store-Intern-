...
function BookDetailPage() {
  ...
  const backHref = typeof location.state?.fromList === "string" ? location.state.fromList : "/books";
  ...
  useEffect(() => {
    ...
    getBookById(idNum)
      ...
      .catch(() => {
        if (!cancelled) {
          setBook(null);
          setError("Không tìm thấy sách hoặc không kết nối được máy chủ.");
        }
      })
      .finally(() => {
      ...
      <div className="books-page bg-light pb-5">
        <div className="container py-5">
          <div className="card border-0 shadow-sm rounded-4 p-5 text-center">
            ...
            <h5 className="fw-bold mb-0">Đang tải chi tiết sách...</h5>
          </div>
        </div>
      </div>
    );
        ...
        <div className="container py-4">
          <nav aria-label="breadcrumb" className="mb-4">
            <ol className="breadcrumb mb-0">
              ...
              <li className="breadcrumb-item"><Link to={backHref} className="text-decoration-none text-dark fw-medium">Sách</Link></li>
          ...
          <div className="card border-0 shadow-sm rounded-4 p-5 text-center">
            <i className="bi bi-book display-4 text-muted mb-3" />
            <h3 className="fw-bold mb-2">{error || "Không tìm thấy sách hoặc không kết nối được máy chủ."}</h3>
            <p className="text-muted mb-4">URL: /books/{idParam}</p>
            <Link to={backHref} className="btn btn-success rounded-pill px-5 fw-bold">Về danh sách sách</Link>
          </div>
        </div>
      </div>
    );
  ...
  const { rating, reviews } = shopSignals(book);
  ...
  const categoryName = bookCategoryName(book) || "Sách";
  const author = book.author || "Đang cập nhật";
  const publisher = book.publisher || "Đang cập nhật";
  const publicationYear = book.publicationYear || "Đang cập nhật";
      ...
      <div className="container py-4">
        <nav aria-label="breadcrumb" className="mb-4">
          <ol className="breadcrumb mb-0">
            <li className="breadcrumb-item"><Link to="/" className="text-decoration-none text-dark fw-medium">Trang chủ</Link></li>
            <li className="breadcrumb-item"><Link to={backHref} className="text-decoration-none text-dark fw-medium">Sách</Link></li>
            <li className="breadcrumb-item active text-truncate" style={{ maxWidth: "220px" }} aria-current="page">{book.title}</li>
          </ol>
        </nav>
        ...
        <div className="card border-0 shadow-sm rounded-4 overflow-hidden mb-5">
          <div className="row g-0">
            <div className="col-lg-5 bg-white p-4 p-lg-5 border-end">
              <div className="position-relative">
                ...
                {book.stockQuantity > 0 && book.stockQuantity <= 5 && (
                  <span className="position-absolute top-0 start-0 m-3 badge rounded-pill bg-warning text-dark fw-bold px-3 py-2 shadow-sm">Sắp hết · {book.stockQuantity}</span>
                )}
                {outOfStock && (
            ...
            <div className="col-lg-7 p-4 p-lg-5 bg-white">
              ...
              <h1 className="fw-bold mb-3 text-dark">{book.title}</h1>

              <div className="text-muted mb-3 d-flex flex-wrap gap-4">
                <div>
                  <i className="bi bi-person me-2"></i>
                  Tác giả: <span className="fw-semibold text-dark">{author}</span>
                </div>
                <div>
                  <i className="bi bi-building me-2"></i>
                  NXB: <span className="fw-semibold text-dark">{publisher}</span>
                </div>
              </div>
              ...
              <div className={`alert rounded-4 fw-bold mb-4 ${isAvailable ? "alert-success border-success-subtle" : "alert-danger border-danger-subtle"}`}>
                {isAvailable ? (
                  <><i className="bi bi-check-circle me-2"></i>Còn {stockQuantity} cuốn trong kho — giao nhanh 2–4 ngày</>
                ) : (
                  <><i className="bi bi-x-circle me-2"></i>Sách hiện đang hết hàng</>
                )}
              </div>
              ...
              <div>
                <h4 className="fw-bold mb-3">Giới thiệu sách</h4>
                <p className="text-muted mb-0" style={{ lineHeight: 1.8, whiteSpace: "pre-wrap" }}>
                  {book.description || "Cuốn sách này thuộc hệ thống nhà sách Book-Store."}
                </p>
              </div>
              ...
              <div>
                ...
                <div className="row g-3">
                  ...
                  <div className="col-md-6">
                    <div className="bg-light rounded-4 p-3 h-100">
                      <div className="text-muted small">Nhà xuất bản</div>
                      <div className="fw-bold">{publisher}</div>
                    </div>
                  </div>
                  <div className="col-md-6">
                    <div className="bg-light rounded-4 p-3 h-100">
                      <div className="text-muted small">Năm xuất bản</div>
                      <div className="fw-bold">{publicationYear}</div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <form onSubmit={handleSubmitReview} className="bg-light rounded-4 p-4 shadow-sm mb-5">
                ...
                <div className="mb-3">
                  <div className="text-muted mb-2">Chất lượng sách</div>
                ...
                <textarea className="form-control border-0 rounded-4 p-3 mb-3" rows="4" placeholder="Hãy chia sẻ cảm nhận của bạn về cuốn sách này..." value={comment} onChange={(e) => setComment(e.target.value)}></textarea>
              ...
              <div className="d-flex flex-column gap-4">
                {loadingReviews ? (
                  ...
                  <div className="text-center py-4 bg-light rounded-4">
                    <p className="text-muted mb-0">Chưa có đánh giá nào cho sách này.</p>
                  </div>
                ) : (
                  reviewsList.map((rev) => (
