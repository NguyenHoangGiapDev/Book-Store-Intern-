...
import { formatBookPrice, FALLBACK_BOOK_IMAGE, getImageUrl } from "../utils/bookDisplay.js";
...
function StationeryDetailPage() {
  ...
  const backUrl = typeof location.state?.from === "string" ? location.state.from : "/stationery";
  ...
  useEffect(() => {
    ...
    apiRequest(`/stationery/${id}`)
      .then((productData) => {
        setItem(productData);
        fetchReviews();
      })
      .catch((err) => {
        setError(err?.message || "Không tìm thấy sản phẩm văn phòng phẩm");
      })
      .finally(() => {
        setLoading(false);
      });
  ...
  const handleSubmitReview = async (e) => {
    ...
    try {
      const reviewPayload = {
        ...
        bookId: null,
        productTitle: item?.title || "Văn phòng phẩm",
        rating: reviewRating,
      ...
      <div className="books-page bg-light pb-5">
        <div className="container py-5">
          <div className="card border-0 shadow-sm rounded-4 p-5 text-center">
            ...
            <h5 className="fw-bold mb-0">Đang tải chi tiết văn phòng phẩm...</h5>
          </div>
        </div>
      </div>
    );
        ...
        <div className="container py-4">
          <nav aria-label="breadcrumb" className="mb-4">
            <ol className="breadcrumb mb-0">
              ...
              <li className="breadcrumb-item"><Link to={backUrl} className="text-decoration-none text-dark fw-medium">Văn phòng phẩm</Link></li>
          ...
          <button type="button" className="btn btn-link text-success fw-bold p-0 mb-4 text-decoration-none" onClick={() => navigate(backUrl)}>
            <i className="bi bi-arrow-left me-2"></i> Quay lại danh sách văn phòng phẩm
          </button>
          ...
          <div className="card border-0 shadow-sm rounded-4 p-5 text-center">
            ...
            <p className="text-muted mb-4">URL: /stationery/{id}</p>
  ...
  const title = item.title || "Sản phẩm văn phòng phẩm";
  const categoryName = item.categoryName || "Văn phòng phẩm";
  const brand = item.brand || item.manufacturer || "Chưa cập nhật";
  const stockQuantity = Number(item.stockQuantity || 0);
  const outOfStock = stockQuantity === 0;
  const isAvailable = stockQuantity > 0;
  const avgRating = 4.5; // Giả định trung bình 4.5 sao
        ...
        <div className="card border-0 shadow-sm rounded-4 overflow-hidden mb-5">
          <div className="row g-0">
            <div className="col-lg-5 bg-white p-4 p-lg-5 border-end">
              <div className="position-relative">
                ...
                {item.stockQuantity > 0 && item.stockQuantity <= 5 && (
                  <span className="position-absolute top-0 start-0 m-3 badge rounded-pill bg-warning text-dark fw-bold px-3 py-2 shadow-sm">Sắp hết · {item.stockQuantity}</span>
                )}
                {outOfStock && (
            ...
            <div className="col-lg-7 p-4 p-lg-5 bg-white">
              ...
              <div>
                <h4 className="fw-bold mb-3">Giới thiệu sản phẩm</h4>
                <p className="text-muted mb-0" style={{ lineHeight: 1.8, whiteSpace: "pre-wrap" }}>
                  {item.description || "Sản phẩm văn phòng phẩm thuộc hệ thống nhà sách Book-Store."}
                </p>
              </div>
              ...
              <div className="d-flex flex-column flex-xl-row justify-content-between gap-3 mb-4">
                ...
                <div className="d-flex align-items-center gap-2">
                  {renderStars(avgRating)}
                  <span className="fw-bold fs-5">{avgRating}/5</span>
                  <span className="text-muted">({reviews.length} đánh giá)</span>
                </div>
              </div>
              ...
              <div className="d-flex flex-column gap-4">
                {loadingReviews ? (
                  ...
                  <div className="text-center py-4 bg-light rounded-4">
                    <p className="text-muted mb-0">Chưa có đánh giá nào được duyệt cho sản phẩm này.</p>
                  </div>
                ) : (
                  reviews.map((rev) => (
