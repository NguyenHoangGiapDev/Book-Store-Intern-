...
import { formatBookPrice, FALLBACK_BOOK_IMAGE, getImageUrl, shopSignals } from "../utils/bookDisplay.js";
...
function SchoolSupplyDetailPage() {
  const { id } = useParams();
  ...
  const loadDetail = async () => {
    try {
      ...
      console.error("Load school supply detail error:", err);
      setError(err?.message || "Không thể tải chi tiết đồ dùng học tập.");
      setItem(null);
    } finally {
      setLoading(false);
    }
  ...
  const handleSubmitReview = async (e) => {
    ...
    try {
      const reviewPayload = {
        ...
        bookId: null,
        productTitle: item?.title || item?.Title || "Đồ dùng học tập",
        rating: reviewRating,
      ...
      setReviewRating(5);
      setReviewContent("");
      loadDetail(); // Reload everything to update reviews
    } catch (err) {
    ...
    try {
      await apiRequest(`/carts/items`, {
        ...
        body: JSON.stringify({
          ...
          SchoolSupplyId: Number(id),
          Quantity: 1
        })
      });

      window.dispatchEvent(new Event('cartUpdated'));
      showToast(`Đã thêm "${item.title || item.Title}" vào giỏ hàng!`, "success");
    } catch (err) {
      ...
      <div className="books-page bg-light pb-5">
        <div className="container py-5">
          <div className="card border-0 shadow-sm rounded-4 p-5 text-center">
            ...
            <h5 className="fw-bold mb-0">Đang tải chi tiết đồ dùng học tập...</h5>
          </div>
        </div>
      </div>
    );
        ...
        <div className="container py-4">
          <nav aria-label="breadcrumb" className="mb-4">
            <ol className="breadcrumb mb-0">
              ...
              <li className="breadcrumb-item"><Link to={backUrl} className="text-decoration-none text-dark fw-medium">Đồ dùng học tập</Link></li>
          ...
          <div className="card border-0 shadow-sm rounded-4 p-5 text-center">
            <i className="bi bi-backpack display-4 text-muted mb-3" />
            <h3 className="fw-bold mb-2">{error || "Không tìm thấy sản phẩm hoặc không kết nối được máy chủ."}</h3>
            <p className="text-muted mb-4">URL: /school-supplies/{id}</p>
            <Link to={backUrl} className="btn btn-success rounded-pill px-5 fw-bold">Về danh sách</Link>
          </div>
        </div>
      </div>
    );
  ...
  const { rating, reviews: reviewsCount } = shopSignals(item);
  const title = item.title || item.Title || "Đồ dùng học tập";
  const brand = item.brand || item.Brand || item.manufacturer || item.Manufacturer || "Đang cập nhật";
  const categoryName = item.categoryName || item.CategoryName || "Đồ dùng học tập";
  const description = item.description || item.Description || "Sản phẩm đồ dùng học tập chất lượng cao.";
  const imageUrl = item.imageUrl || item.ImageUrl;
  const price = Number(item.price ?? item.Price ?? 0);
  const stockQuantity = Number(item.stockQuantity ?? item.StockQuantity ?? 0);
  ...
  return (
    <div className="books-page bg-light pb-5">
      <div className="container py-4">
        ...
        <button type="button" className="btn btn-link text-success fw-bold p-0 mb-4 text-decoration-none" onClick={() => navigate(backUrl)}>
          <i className="bi bi-arrow-left me-2"></i> Quay lại danh sách đồ dùng học tập
        </button>
              ...
              <div className="position-relative">
                {imageUrl ? (
                  ...
                  <div className="d-flex align-items-center justify-content-center bg-light rounded-4" style={{ height: "520px" }}>
                    <i className="bi bi-backpack display-1 text-muted"></i>
                  </div>
                )}
...
export default SchoolSupplyDetailPage;